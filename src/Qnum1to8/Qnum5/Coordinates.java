...
public class Coordinates {
    ...
    public int[][] getKeyCoordinates(int[][] height) {
        ...
        for (int[] rect : height) {
            ...
            map.put(rect[1], Math.max(map.getOrDefault(rect[1], 0), 0 - rect[2]));
        }

        // Keep track of the current height while iterating through the map
        int currHeight = 0;
        int[][] res = new int[2 * map.size()][2];
        int i = 0;
        for (Map.Entry<Integer, Integer> entry : map.entrySet()) {
            ...
            if (h >= 0 && h != currHeight) {
                ...
                i++;
            } else if (h < 0 && -h == currHeight) {
                res[i][0] = x;
                res[i][1] = currHeight = 0;
                i++;
            }
        }
